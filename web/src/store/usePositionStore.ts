...
import { Assessment, Candidate, Criterion, CustomError, Position } from '@/types';
...
export const usePositionStore = create<PositionState>((set, get) => ({
  ...
  selectPosition: (position: Position) => {
    set({ selectedPosition: position }), get().setPositionDetail(position.id);
  },
        ...
        set({
          selectedPosition: {
            ...
            candidates: assignCriteriaValueForCandidates(
              ...
              res.data.payload.checklist ? res.data.payload.checklist.criteria : []
            ),
          },
        });
      } else {
